...
import { cachedResolveUsername } from "@/lib/username-resolver";

// 保守方案：保留 force-dynamic 确保 restrictions 实时查询
// 只对 resolveUsername 做缓存，减少重复 DB 查询
...
type CurrentProfile = Extract<
  Awaited<ReturnType<typeof cachedResolveUsername>>,
  { type: "current" }
>["profile"];

export async function generateMetadata({ params }: PublicProfilePageProps): Promise<Metadata> {
  const { username } = await params;
  const result = await cachedResolveUsername(username);
    ...
    return (
      <StatePage
        ...
        action={
          <Link
            href="/login"
            className="inline-flex min-h-11 items-center justify-center rounded-full bg-[var(--ui-success)] px-6 text-sm font-black text-white"
          >
            登录后台验证
          </Link>
        }
