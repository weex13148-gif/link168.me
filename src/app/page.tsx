...
export default function Home() {
  return (
    <div className="ui-page dark-public">
      <AppHeader />
      <main>
        <section className="border-b border-[var(--ui-line)] bg-[var(--ui-surface)] py-12 sm:py-16 lg:py-20">
          <div className="ui-container grid items-center gap-10 lg:grid-cols-[minmax(0,1fr)_370px] lg:gap-16">
            ...
            {/* 专属链接输入框 */}
            <div className="ui-container mt-8">
              <HomeHandleForm />
            </div>
        </section>
        ...
        <section id="pricing" className="border-t border-[var(--ui-line)] bg-[var(--ui-surface)] py-14 sm:py-16">
          <div className="ui-container">
            <div className="max-w-3xl"><p className="ui-eyebrow">收费方案</p><h2 className="ui-title mt-3 text-3xl sm:text-4xl">先免费使用，再按经营阶段升级</h2><p className="ui-muted mt-3 leading-7">当前在线收款仅支持支付宝，微信支付后续开放。所有付费套餐按年开通，企业版支持在线购买与联系销售定制两种方案。</p></div>
            ...
            {/* AI 接待加油包 */}
            <div className="mt-8 rounded-[var(--ui-radius-lg)] border border-dashed border-[var(--ui-line)] bg-[var(--ui-surface)] p-5 sm:p-6">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <h3 className="text-lg font-black text-[var(--ui-ink)]">AI 接待通用加油包</h3>
                  <p className="ui-muted mt-1 text-sm leading-6">所有用户可买，所有用户同价。只增加 AI 接待次数，不解锁会员权益。AI 加油包支付正在开通中，当前暂不支持在线购买。</p>
                  <ul className="mt-3 grid gap-2 text-sm">
                    <li className="flex items-start gap-2"><Check className="mt-0.5 size-4 shrink-0 text-[var(--ui-brand)]" /><span>100 次 AI 接待会话额度</span></li>
                    <li className="flex items-start gap-2"><Check className="mt-0.5 size-4 shrink-0 text-[var(--ui-brand)]" /><span>所有用户可购买，不限套餐</span></li>
                    <li className="flex items-start gap-2"><Check className="mt-0.5 size-4 shrink-0 text-[var(--ui-brand)]" /><span>购买后立即生效，90 天内有效</span></li>
                  </ul>
                </div>
                <div className="flex shrink-0 flex-col items-start gap-2 sm:items-end">
                  <p className="text-2xl font-black text-[var(--ui-brand-hover)]">¥9.9 <span className="text-sm font-normal text-[var(--ui-muted)]">/ 100 次</span></p>
                  <button disabled className="ui-button-secondary min-h-10 px-5 text-sm disabled:opacity-45">支付开通中</button>
                </div>
              </div>
            </div>
          </div>
