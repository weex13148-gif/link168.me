  ...
  return (
    <main className="relative mx-auto flex min-h-dvh w-full max-w-md items-center overflow-hidden px-4 py-8">
      ...
      <section className="w-full rounded-[28px] border border-[#E8DCCB] bg-[#FFFDF8]/90 p-6 shadow-[0_24px_70px_rgba(86,68,46,0.12)] backdrop-blur">
        ...
        <h1 className="mt-6 text-3xl font-black text-[#2B241E]">修改密码</h1>
        ...
        <form onSubmit={onSubmit} className="mt-6 grid gap-4">
          ...
          <label className="flex items-center gap-3 rounded-2xl border border-[#E8DCCB] bg-[#F7F1E7] p-4 cursor-pointer transition hover:bg-[#FFFDF8] hover:border-[#6F8F4E]">
            <input
              type="checkbox"
              checked={logoutOtherDevices}
              onChange={(event) => setLogoutOtherDevices(event.target.checked)}
              className="size-5 rounded border-[#E8DCCB] text-[#6F8F4E] accent-[#6F8F4E]"
            />
            <div>
              <span className="block text-sm font-black text-[#2B241E]">退出其他设备</span>
              <span className="text-xs text-[#7A6D5E]">修改密码后，其他已登录设备将被强制登出</span>
            </div>
          </label>
