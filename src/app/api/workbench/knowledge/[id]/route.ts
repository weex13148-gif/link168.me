...
async function getPersonalDoc(userId: string, id: string) {
...
export async function GET(request: Request, context: RouteContext) {
  ...
  const doc = await getPersonalDoc(user.id, id);
  if (!doc) {
    return NextResponse.json({ success: false, error: "资料不存在或无权访问。" }, { status: 404 });
  }
  return NextResponse.json({ success: true, doc: toDto(doc) });
}
...
export async function PUT(request: Request, context: RouteContext) {
  ...
  const existing = await getPersonalDoc(user.id, id);
  if (!existing) {
    return NextResponse.json({ success: false, error: "资料不存在或无权访问。" }, { status: 404 });
  }
  ...
  if (typeof body.allowAiCitation === "boolean") data.allowAiCitation = body.allowAiCitation;
