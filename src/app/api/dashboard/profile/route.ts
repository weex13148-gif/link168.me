...
import { isAutoGeneratedHandle, isPlaceholderHandle } from "@/lib/handle";
...
export async function PUT(request: Request) {
  ...
  if (isPublicValue !== undefined) {
    ...
    if (isPublicValue) updateData.firstPublishedAt = { set: new Date() };
  }
  ...
  const upsertData = {
    ...
    create: {
      ...
      firstPublishedAt: isPublicValue ? new Date() : null,
  ...
  try {
    profile = await db.$transaction(async (tx) => {
      ...
      if (isPublicValue) {
        const effectiveDisplayName = hasField(body, "displayName")
          ? displayNameSafe
          : existingProfile?.displayName;
        const effectiveBio = hasField(body, "bio") ? bioSafe : existingProfile?.bio;
        const username = existingProfile?.username || "";
        const profileReady =
          Boolean(effectiveDisplayName)
          && Boolean(effectiveBio)
          && !isPlaceholderHandle(username)
          && !isAutoGeneratedHandle(username)
          && Boolean(existingProfile?.links.length);
        if (!profileReady) {
          throw new PublishValidationError();
        }
      }
      if (existingProfile?.firstPublishedAt && isPublicValue) {
        delete updateData.firstPublishedAt;
      }
      return tx.profile.upsert(upsertData);
    });
  } catch (error) {
    if (error instanceof PublishValidationError) {
      return NextResponse.json(
        { success: false, error: "发布前请先完成经营名称、业务介绍、公开地址和一个客户动作。" },
        { status: 400 },
      );
    }
    throw error;
