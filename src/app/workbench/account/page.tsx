...
export default async function WorkbenchAccountPage() {
  ...
  const rows = [
    { label: "登录邮箱", value: user.email, icon: Mail, tone: "bg-[#DDE8CD] text-[#3F5F31]", action: null },
    {
      ...
      icon: Mail,
      tone: user.emailVerified ? "bg-[#DDE8CD] text-[#3F5F31]" : "bg-[#FFE6E2] text-[#B42318]",
      action: null,
    },
    { label: "登录密码", value: "已设置 · 定期更换更安全", icon: Key, tone: "bg-[#EAF3FF] text-[#2563EB]", action: "修改密码" },
    { label: "两步验证", value: "未开启 · 建议开启", icon: Smartphone, tone: "bg-[#F6E7C8] text-[#8C612E]", action: "开启" },
    { label: "通知设置", value: "仅接收重要经营提醒", icon: Bell, tone: "bg-[#FFE6E2] text-[#B42318]", action: "配置" },
  ];
    ...
    >
      <section className="rounded-[28px] border border-[#E8DCCB] bg-white p-5 shadow-sm sm:p-6">
        <div className="flex flex-wrap items-center gap-4">
          <span className="grid size-16 place-items-center rounded-3xl bg-[#DDE8CD] text-2xl font-black text-[#2B241E]">
            {user.email.charAt(0).toUpperCase()}
          </span>
          <div>
            <p className="text-lg font-black text-[#2B241E]">{user.email}</p>
            <p className="text-xs text-[#7A6D5E]">{user.email} · 注册时间 {createdDate}</p>
          </div>
          <span className={`ml-auto rounded-full px-3 py-1.5 text-xs font-black ${isMember ? "bg-[#6F8F4E] text-white" : "bg-[#F7F1E7] text-[#3F5F31]"}`}>
            {planName}
          </span>
        </div>
      </section>
      ...
      <section className="mt-6 grid gap-3">
        {rows.map((row) => {
          ...
          return (
            <div
              key={row.label}
              className="flex flex-wrap items-center gap-4 rounded-[24px] border border-[#E8DCCB] bg-white p-4 shadow-sm sm:p-5"
            >
              <span className={`grid size-11 shrink-0 place-items-center rounded-2xl ${row.tone}`}>
                <Icon aria-hidden className="size-5" />
              </span>
              <div className="min-w-0 flex-1">
                <p className="text-sm font-black text-[#2B241E]">{row.label}</p>
                <p className="truncate text-xs text-[#7A6D5E]">{row.value}</p>
              </div>
              {row.action && (
                <span className="rounded-full bg-[#2B241E] px-4 py-2 text-xs font-black text-white">
                  {row.action}
                </span>
              )}
            </div>
      ...
      <section className="mt-6 rounded-[28px] border border-[#E8DCCB] bg-white p-5 shadow-sm sm:p-6">
        <div className="flex items-center gap-2">
          <ShieldCheck aria-hidden className="size-4 text-[#6F8F4E]" />
          <p className="text-sm font-black text-[#3F5F31]">登录设备与历史</p>
        </div>
        <p className="mt-1 text-xs text-[#7A6D5E]">查看最近登录的设备，如发现异常请立即修改密码。</p>
