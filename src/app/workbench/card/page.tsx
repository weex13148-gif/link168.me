...
export default async function WorkbenchCardPage() {
  ...
  if (!isPaid && !isAdmin) redirect("/console/card");
      ...
      <section className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
        <div className="grid gap-4">
          <div className="rounded-[28px] border border-[#E8DCCB] bg-white p-5 shadow-sm sm:p-6">
            <div className="flex items-center justify-between gap-3"><div><p className="text-sm font-black text-[#3F5F31]">名片基础信息</p><p className="mt-1 text-xs text-[#7A6D5E]">访客看到的基础资料会同步用于 AI 回答。</p></div><Link href="/console/card" className="link168-button-press inline-flex min-h-10 items-center gap-2 rounded-full bg-[#DDE8CD] px-4 text-xs font-black text-[#3F5F31]"><Pencil aria-hidden className="size-4" /> 在 Dashboard 编辑</Link></div>
          ...
          <div className="rounded-[28px] border border-[#E8DCCB] bg-white p-5 shadow-sm sm:p-6">
            <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-black text-[#3F5F31]">名片链接与组件</p><p className="mt-1 text-xs text-[#7A6D5E]">共 {profile.links.length} 个链接，{activeLinks.length} 个已启用。</p></div><Link href="/console/card" className="link168-button-press inline-flex min-h-10 items-center gap-2 rounded-full bg-[#6F8F4E] px-4 text-xs font-black text-white"><Plus aria-hidden className="size-4" /> 管理链接</Link></div>
            {profile.links.length > 0 ? <ul className="mt-4 grid gap-3">{profile.links.slice(0, 6).map((link, index) => <li key={link.id} className="flex flex-wrap items-center gap-3 rounded-[24px] border border-[#E8DCCB] bg-[#F7F1E7] p-4"><span className="grid size-9 place-items-center rounded-2xl bg-white text-xs font-black text-[#7A6D5E] ring-1 ring-[#E8DCCB]">{index + 1}</span><div className="min-w-0 flex-1"><p className="truncate text-sm font-black text-[#2B241E]">{link.title}</p><p className="truncate text-xs text-[#7A6D5E]">{link.url}</p></div><span className={`rounded-full px-3 py-1.5 text-xs font-bold ${link.isActive ? "bg-[#DDE8CD] text-[#3F5F31]" : "bg-[#F7F1E7] text-[#7A6D5E] ring-1 ring-[#E8DCCB]"}`}>{link.isActive ? "已启用" : "已隐藏"}</span></li>)}</ul> : <div className="mt-6 text-center"><p className="text-sm text-[#7A6D5E]">还没有链接，先去 Dashboard 添加一个。</p><Link href="/console/card" className="mt-3 inline-flex min-h-10 items-center gap-2 rounded-full bg-[#6F8F4E] px-4 text-xs font-black text-white"><Plus aria-hidden className="size-4" /> 添加第一个链接</Link></div>}
          </div>
        </div>
        ...
        <div className="grid gap-4 content-start">
          <div className="rounded-[28px] border border-[#E8DCCB] bg-white p-5 shadow-sm sm:p-6">
            ...
            <div className="mt-4 rounded-3xl bg-gradient-to-br from-[#DDE8CD] via-[#F7F1E7] to-[#EAF3FF] p-5">
              ...
              <div className="mt-4 flex flex-wrap items-center gap-2"><Link href={publicUrl} target="_blank" rel="noopener noreferrer" className="link168-button-press inline-flex min-h-10 items-center gap-2 rounded-full bg-[#2B241E] px-4 text-xs font-black text-white"><Share2 aria-hidden className="size-4" /> 查看公开页</Link><Link href="/console/card" className="link168-button-press inline-flex min-h-10 items-center gap-2 rounded-full bg-white px-4 text-xs font-black text-[#3F5F31] ring-1 ring-[#E8DCCB]"><Palette aria-hidden className="size-4" /> 编辑主题</Link></div>
            </div>
          </div>
          <div className="rounded-[28px] border border-[#E8DCCB] bg-white p-5 shadow-sm"><p className="text-sm font-black text-[#3F5F31]">AI 名片能力</p><p className="mt-1 text-xs text-[#7A6D5E]">AI 客服会基于你的名片和产品资料回答访客咨询。</p><ul className="mt-3 grid gap-2 text-sm">{[{ label: "AI 自我介绍", done: !!profile.bio }, { label: "产品推荐", done: true }, { label: "线索收集", done: true }].map((item) => <li key={item.label} className="flex items-center justify-between rounded-2xl bg-[#F7F1E7] px-4 py-3"><span className="font-bold text-[#2B241E]">{item.label}</span><span className={`rounded-full px-3 py-1 text-xs font-black ${item.done ? "bg-[#DDE8CD] text-[#3F5F31]" : "bg-[#FFE6E2] text-[#B42318]"}`}>{item.done ? "已启用" : "需补充资料"}</span></li>)}</ul></div>
        </div>
      </section>
    </WorkbenchShell>
  );
