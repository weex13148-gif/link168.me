...
export default function WorkbenchEnterprisePage() {
      ...
      <WorkbenchShell eyebrow="Enterprise" title="企业工作空间" subtitle="管理企业资料与团队成员。">
        <div className="rounded-[28px] border border-[#E8DCCB] bg-white p-8 text-center">
          ...
          <button
            ...
            className="mt-4 inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl bg-[#6F8F4E] px-5 text-sm font-black text-white hover:bg-[#5E7F3F]"
          >
            重新加载
          </button>
        </div>
    ...
    return (
      <WorkbenchShell eyebrow="Enterprise" title="企业工作空间" subtitle="创建或加入一个工作空间开始协作。">
        <div className="rounded-[28px] border border-[#E8DCCB] bg-white p-8 text-center">
          {workspaces.length === 0 ? (
            <>
              <p className="text-sm text-[#7A6D5E]">你还没有任何工作空间</p>
              <button
            ...
            <>
              <p className="text-sm text-[#7A6D5E]">选择一个工作空间</p>
              <WorkspaceSwitcher
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 flex gap-2 border-b border-[#E8DCCB]">
        <button
          onClick={() => setActiveTab("profile")}
          className={`relative px-4 py-2 text-sm font-black transition ${
            activeTab === "profile"
              ? "text-[#2B241E]"
              : "text-[#7A6D5E] hover:text-[#2B241E]"
          }`}
        >
          企业资料
          {activeTab === "profile" && (
            <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#6F8F4E]" />
          )}
        </button>
        <button
        ...
        >
          成员管理
          <span className="ml-2 rounded-full bg-[#F7F1E7] px-2 py-0.5 text-xs font-black text-[#7A6D5E]">
            {members.length}
          </span>
          {activeTab === "members" && (
            <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#6F8F4E]" />
