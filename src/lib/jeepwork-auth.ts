...
async function getJeepworkCapabilities(user: JeepworkUser) {
...
async function getJeepworkUserByToken(
  ...
  try {
    const { capabilities } = await getJeepworkCapabilities(session.user);
    if (!capabilities.canEnterJeepwork) return null;
  } catch {
    return null;
  }
...
async function getUserFromRequest(
  ...
  const token = tokenMatch
    ? tokenMatch.slice(JEEPWORK_COOKIE_NAME.length + 1)
    : undefined;
...
export async function requireJeepworkAdmin(
  request: Request,
): Promise<NextResponse | null> {
  return requireJeepworkSuperAdmin(request);
}

export async function requireJeepworkSuperAdmin(
  request: Request,
): Promise<NextResponse | null> {
...
export async function getJeepworkSessionUser(
  request: Request,
): Promise<{ id: string; email: string; role: string } | null> {
...
async function rateLimitCheck(
  ...
  const g = globalThis as unknown as {
    __jeepworkRateLimit?: Record<string, { count: number; resetAt: number }>;
  };
...
export async function jeepworkLoginHandler(
  request: Request,
  emailRaw: string,
  passwordRaw: string,
  ...
  if (!shouldBypassRateLimit()) {
    const ipHit = await rateLimitCheck(
      request,
      "jeepwork:ip",
      IP_RATE_LIMIT_MAX,
    ...
    if (!ipHit.passed) {
      return adminError("RATE_LIMIT", "请求过于频繁，请稍后再试", 429);
    }
  }
  ...
  if (!email || !password) {
    return adminError("BAD_CREDENTIALS", "账号或密码错误", 401);
  }

  if (!shouldBypassRateLimit()) {
    const emailHit = await rateLimitCheck(
      request,
      `jeepwork:email:${email}`,
  ...
  if (user.role !== "super_admin" || !passwordOk) {
    return adminError("BAD_CREDENTIALS", "账号或密码错误", 401);
  }

  let capabilities;
  try {
    ({ capabilities } = await getJeepworkCapabilities(user));
  } catch {
    return adminError(
      "RESTRICTION_SERVICE_UNAVAILABLE",
      "服务暂不可用",
      503,
    );
  }

  if (!capabilities.canEnterJeepwork) {
    return adminError("ACCOUNT_RESTRICTED", "账号当前无法进入后台", 403);
  }
...
export async function jeepworkLogoutHandler(
  request: Request,
): Promise<NextResponse> {
  const cookieHeader = request.headers.get("cookie") || "";
  const tokenMatch = cookieHeader
...
export async function hasOtherSuperAdmin(
