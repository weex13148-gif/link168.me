    ...
    <>
      ...
      >
        ...
        <div className="absolute inset-0 grid place-items-center bg-black/0 transition-colors group-hover:bg-black/20">
          <span className="grid size-10 place-items-center rounded-full bg-black/40 text-[var(--ui-surface)] opacity-0 backdrop-blur-sm transition-opacity group-hover:opacity-100">
      ...
      {open ? (
        ...
        >
          <button
            type="button"
            onClick={() => setOpen(false)}
            className="absolute right-4 top-4 grid size-10 place-items-center rounded-full bg-[var(--ui-surface)]/10 text-[var(--ui-surface)] backdrop-blur-sm hover:bg-[var(--ui-surface)]/20"
            aria-label="关闭"
          >
            <X className="size-5" />
          </button>
