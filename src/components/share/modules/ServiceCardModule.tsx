      ...
      <div className="flex flex-col overflow-hidden rounded-2xl border border-[var(--ui-line)] bg-[var(--ui-surface)] shadow-sm">
        {coverImageUrl ? (
          ...
          <div className="flex aspect-[16/9] w-full items-center justify-center bg-gradient-to-br from-[var(--ui-success-soft)] to-[var(--ui-surface-muted)]">
            <ConciergeBell className="size-12 text-[var(--ui-success)]" />
          </div>
        )}

        <div className="flex flex-1 flex-col p-4">
          {category ? <span className="mb-1 inline-block w-fit rounded-full bg-[var(--ui-success-soft)] px-2 py-0.5 text-[10px] font-black uppercase tracking-wider text-[var(--ui-brand)]">{category}</span> : null}
        ...
        <div className="flex gap-2 border-t border-[var(--ui-line)] p-3">
          {ctaUrl ? <a href={ctaUrl} target="_blank" rel="noopener noreferrer" className="flex-1 rounded-xl border border-[var(--ui-line)] bg-[var(--ui-surface)] px-3 py-2 text-center text-xs font-black text-[var(--ui-ink)] transition hover:bg-[var(--ui-surface-muted)]">{ctaLabel || "查看详情"}</a> : null}
          {allowBooking ? <button type="button" onClick={() => openForm("booking")} className="flex flex-1 items-center justify-center gap-1.5 rounded-xl border border-[var(--ui-success)] bg-[var(--ui-surface)] px-3 py-2 text-xs font-black text-[var(--ui-success)] transition hover:bg-[var(--ui-success-soft)]"><CalendarClock className="size-4" />预约</button> : null}
          <button type="button" onClick={() => openForm("consult")} className="flex flex-1 items-center justify-center gap-1.5 rounded-xl bg-[var(--ui-success)] px-3 py-2 text-xs font-black text-[var(--ui-surface)] transition hover:bg-[var(--ui-success)]"><MessageCircle className="size-4" />咨询</button>
        </div>
      </div>
            ...
            <div className="w-full max-w-md rounded-[30px] border border-[var(--ui-line)] bg-[var(--ui-surface)] p-6 shadow-[0_18px_55px_rgba(86,68,46,0.12)]">
              <div className="mb-4 flex items-center justify-between border-b border-[var(--ui-line)] pb-4">
                <div className="flex min-w-0 items-center gap-3">
                  ...
                  ) : <div className="flex size-12 shrink-0 items-center justify-center rounded-xl bg-[var(--ui-success-soft)]"><ConciergeBell className="size-6 text-[var(--ui-success)]" /></div>}
                  <div className="min-w-0 flex-1"><p className="text-xs font-black text-[var(--ui-success)]">{formType === "booking" ? "预约服务" : "咨询服务"}</p><p className="mt-0.5 truncate text-base font-black text-[var(--ui-ink)]">{name}</p></div>
                </div>
              ...
              <form onSubmit={handleSubmit} className="grid gap-3">
                ...
                {formType === "booking" ? (
                  <div className="grid grid-cols-2 gap-3">
                    <label className="grid gap-1.5 text-sm"><span className="font-black text-[var(--ui-ink)]">期望日期</span><input type="date" value={form.preferredDate} onChange={(event) => setForm((current) => ({ ...current, preferredDate: event.target.value }))} className="rounded-2xl border border-[var(--ui-line)] bg-[var(--ui-surface)] px-4 py-3 text-sm text-[var(--ui-ink)] focus:border-[var(--ui-brand)] focus:outline-none" /></label>
                    <label className="grid gap-1.5 text-sm"><span className="font-black text-[var(--ui-ink)]">期望时间</span><input type="time" value={form.preferredTime} onChange={(event) => setForm((current) => ({ ...current, preferredTime: event.target.value }))} className="rounded-2xl border border-[var(--ui-line)] bg-[var(--ui-surface)] px-4 py-3 text-sm text-[var(--ui-ink)] focus:border-[var(--ui-brand)] focus:outline-none" /></label>
                  </div>
                ) : null}
                <label className="grid gap-1.5 text-sm"><span className="font-black text-[var(--ui-ink)]">备注（选填）</span><textarea value={form.message} onChange={(event) => setForm((current) => ({ ...current, message: event.target.value }))} placeholder="其他需求说明" maxLength={300} rows={2} className="resize-none rounded-2xl border border-[var(--ui-line)] bg-[var(--ui-surface)] px-4 py-3 text-sm text-[var(--ui-ink)] placeholder-[var(--ui-muted)] focus:border-[var(--ui-brand)] focus:outline-none" /></label>
                {error ? <p className="rounded-2xl bg-[var(--ui-danger-soft)] px-4 py-2 text-sm text-[var(--ui-danger)]">{error}</p> : null}
                <button type="submit" disabled={loading || submitted} className="mt-2 inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl bg-[var(--ui-success)] text-sm font-black text-[var(--ui-surface)] transition hover:bg-[var(--ui-success)] disabled:opacity-50">{loading ? <><Loader className="size-4 animate-spin" />提交中...</> : <><MessageCircle className="size-4" />{formType === "booking" ? "提交预约" : "提交咨询"}</>}</button>
              </form>
            </div>
          )}
        </div>
