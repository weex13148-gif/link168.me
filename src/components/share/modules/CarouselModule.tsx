      ...
      {total > 1 ? (
        <>
          <button
            type="button"
            onClick={prev}
            className="absolute left-2 top-1/2 grid size-9 -translate-y-1/2 place-items-center rounded-full bg-black/30 text-[var(--ui-surface)] backdrop-blur-sm hover:bg-black/50"
            aria-label="上一张"
          >
          ...
          <button
            type="button"
            onClick={next}
            className="absolute right-2 top-1/2 grid size-9 -translate-y-1/2 place-items-center rounded-full bg-black/30 text-[var(--ui-surface)] backdrop-blur-sm hover:bg-black/50"
            aria-label="下一张"
          >
          ...
          <div className="absolute bottom-3 left-1/2 flex -translate-x-1/2 items-center gap-1.5">
            {images.map((_, index) => (
              <button
                ...
                type="button"
                onClick={() => goTo(index)}
                className={`h-1.5 rounded-full transition-all ${index === current ? "w-6 bg-[var(--ui-surface)]" : "w-1.5 bg-[var(--ui-surface)]/50"}`}
