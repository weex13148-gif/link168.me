    ...
    >
      ...
      >
        <div className="flex items-center justify-between gap-3">
          <div>
            <p className="text-sm font-black text-[var(--ui-brand)]">二维码</p>
            <h3 id="qrcode-modal-title" className="mt-1 text-xl font-black text-[var(--ui-ink)]">
              {displayName}
            </h3>
          </div>
          <button
        ...
        <div className="mt-5 flex flex-col items-center">
          ...
          <p className="mt-4 text-center text-xs text-[var(--ui-muted)] sm:text-sm">
            扫码访问 @{username} 的主页
          </p>

          <button
            onClick={handleDownload}
            className="link168-button-press mt-5 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-2xl bg-[var(--ui-success)] text-sm font-black text-[var(--ui-surface)] hover:bg-[var(--ui-success)] transition-colors focus:outline-none focus:ring-2 focus:ring-[var(--ui-brand)] focus:ring-offset-2"
          >
            {downloaded ? (
              <>
                <CheckCircle className="size-5" aria-hidden="true" />
                <span className="text-[var(--ui-success)]">已保存</span>
              </>
            ) : (
              <>
