...
function BrandFooter() {
  return (
    <a
      ...
      className="mx-auto mt-5 inline-flex min-h-10 items-center gap-2 rounded-full border border-black/10 bg-[var(--ui-surface)]/80 px-4 py-2 text-sm font-black text-[var(--ui-brand)] shadow-sm transition hover:-translate-y-0.5 hover:bg-[var(--ui-surface)]"
      aria-label="访问 Link168 官网"
    >
      <span className="grid size-7 place-items-center rounded-xl bg-[var(--ui-success)] text-xs font-black text-[var(--ui-surface)]">L</span>
      由 Link168 提供
    </a>
  );
}
...
function ContactForm({
  ...
  return (
    <div className="fixed inset-0 z-[60] grid place-items-center bg-[var(--ui-ink)]/45 p-4" onClick={onClose} role="presentation">
      <section
        className="w-full max-w-sm rounded-[28px] border border-[var(--ui-line)] bg-[var(--ui-surface)] p-6 shadow-[0_24px_80px_rgba(43,36,30,0.22)]"
        role="dialog"
        aria-modal="true"
      ...
      >
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-sm font-black text-[var(--ui-brand)]">联系主页所有者</p>
            <h2 id="contact-title" className="mt-1 text-xl font-black text-[var(--ui-ink)]">@{username}</h2>
          </div>
          <button ref={closeButtonRef} type="button" onClick={onClose} className="grid size-10 place-items-center rounded-xl bg-[var(--ui-surface-muted)]" aria-label="关闭">
            <X className="size-5 text-[var(--ui-muted)]" />
          </button>
        </div>

        {success ? (
          <div className="mt-6 rounded-2xl bg-[var(--ui-success-soft)] p-6 text-center">
            <CheckCircle className="mx-auto size-12 text-[var(--ui-success)]" />
            <p className="mt-3 font-black text-[var(--ui-brand)]">提交成功</p>
            <p className="mt-2 text-sm text-[var(--ui-brand)]">主页所有者会在客户线索中查看你的信息。</p>
          </div>
        ) : (
          <form onSubmit={submit} className="mt-5 grid gap-3">
            {products.length ? (
              <label className="grid gap-1.5 text-sm">
                ...
                <select value={form.productId} onChange={(event) => setForm((current) => ({ ...current, productId: event.target.value }))} className="rounded-xl border border-[var(--ui-line)] bg-[var(--ui-surface)] px-4 py-3">
            ...
            <label className="grid gap-1.5 text-sm">
              ...
              <input value={form.name} onChange={(event) => setForm((current) => ({ ...current, name: event.target.value }))} maxLength={50} placeholder="如何称呼你" className="rounded-xl border border-[var(--ui-line)] bg-[var(--ui-surface)] px-4 py-3" />
            </label>
            <label className="grid gap-1.5 text-sm">
              <span className="font-black">邮箱、电话或微信</span>
              <input value={form.contact} onChange={(event) => setForm((current) => ({ ...current, contact: event.target.value }))} maxLength={100} placeholder="方便联系你的方式" className="rounded-xl border border-[var(--ui-line)] bg-[var(--ui-surface)] px-4 py-3" />
            </label>
            <label className="grid gap-1.5 text-sm">
              <span className="font-black">留言（选填）</span>
              <textarea value={form.message} onChange={(event) => setForm((current) => ({ ...current, message: event.target.value }))} rows={3} maxLength={500} placeholder="你想咨询什么内容？" className="resize-none rounded-xl border border-[var(--ui-line)] bg-[var(--ui-surface)] px-4 py-3" />
            </label>
            {error ? <p className="rounded-xl bg-[var(--ui-danger-soft)] px-4 py-3 text-sm font-bold text-[var(--ui-danger)]">{error}</p> : null}
            <button type="submit" disabled={loading} className="mt-1 inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-[var(--ui-success)] text-sm font-black text-[var(--ui-surface)] disabled:opacity-50">
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-6 left-5 z-40 flex min-h-12 items-center gap-2 rounded-full border border-[var(--ui-success)] bg-[var(--ui-surface)] px-4 text-sm font-black text-[var(--ui-brand)] shadow-lg hover:bg-[var(--ui-success-soft)]"
        aria-label="联系主页所有者"
      >
