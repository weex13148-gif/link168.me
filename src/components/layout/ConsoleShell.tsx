...
export default function ConsoleShell({
  ...
  return (
    <div className="dark-public min-h-dvh w-full overflow-x-hidden bg-[var(--ui-page)]">
        ...
        <aside className="hidden lg:sticky lg:top-4 lg:block lg:h-[calc(100dvh-32px)] lg:w-[248px] lg:shrink-0">
          <div className="flex h-full flex-col rounded-[28px] border border-[var(--ui-line)] bg-[var(--ui-surface)] p-5 shadow-sm">
            <Link href="/console" className="flex items-center gap-2">
              <span className="grid size-9 place-items-center rounded-2xl bg-[var(--ui-success)] text-base font-black text-white">
                L
              </span>
            ...
            <div className="mt-auto rounded-2xl border border-[var(--ui-line)] bg-[var(--ui-surface-muted)] p-4">
              <p className="text-xs font-black text-[var(--ui-muted)]">五个一级入口</p>
              <p className="mt-1 text-sm font-bold leading-6 text-[var(--ui-brand)]">
                首页 · 名片 · 客户 · AI · 我的
              </p>
            </div>
          </div>
        ...
        <main className="min-w-0 flex-1 pb-24 lg:pb-0">
          <div className="mb-4 flex min-w-0 items-center gap-2 lg:hidden">
            <Link href="/console" className="flex min-w-0 items-center gap-2">
              <span className="grid size-8 shrink-0 place-items-center rounded-xl bg-[var(--ui-success)] text-sm font-black text-white">
                L
              </span>
          ...
          <header className="min-w-0 rounded-[24px] border border-[var(--ui-line)] bg-[var(--ui-surface)] p-4 shadow-sm sm:rounded-[28px] sm:p-6">
            {eyebrow ? (
              <p className="break-words text-xs font-black uppercase tracking-[0.16em] text-[var(--ui-success)] sm:tracking-[0.2em]">
                {eyebrow}
              </p>
            ) : null}
      ...
      <nav
        className="fixed inset-x-0 bottom-0 z-40 border-t border-[var(--ui-line)] bg-[var(--ui-surface)]/96 px-2 py-1 backdrop-blur lg:hidden safe-area-pb"
        aria-label="手机端控制台导航"
      >
            ...
            return (
              <Link
                ...
                className={`flex min-h-[58px] min-w-0 flex-col items-center justify-center gap-1 rounded-xl px-1 py-2 text-[10px] font-black transition ${
                  active
                    ? "bg-[var(--ui-success-soft)] text-[var(--ui-brand)]"
