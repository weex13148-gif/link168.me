...
const COPY: Partial<Record<ShowcaseV2SectionKey, { title: string; body: string }>> = {
  opening: {
    title: "Link168：面向中文创作者和小商家的个人数字名片平台",
    body: "用一个公开主页集中展示内容、服务、联系方式和二维码。核心主页闭环已经完成，AI 与商业化能力按实际进度标注。",
  },
  solution: {
    title: "先解决展示与分享，再逐步扩展经营与 AI 能力",
    body: "Link168 先完成注册、主页编辑、真实链接、公开主页和二维码分享，再在同一产品结构中扩展数据、企业资料和 AI 助理。",
  },
  productDemo: {
    title: "评委可以直接打开真实产品页面",
    body: "这里不使用产品截图代替功能。每个入口都指向仓库中的真实页面，并明确标记是否需要登录或仍处于内测阶段。",
  },
  aiAssistants: {
    title: "五个可演示 AI Agent，加一个规划中的销售顾问",
    body: "财税、法务、市场调研、设计和社媒运营已具备比赛演示入口；销售顾问保留为下一阶段能力，不伪装成已完成。",
  },
  businessModel: {
    title: "免费主页获客，会员和企业服务变现",
    body: "免费版负责降低使用门槛；会员版提供高级主题、数据和二维码能力；企业版承载企业资料、AI 助理和团队服务。",
  },
  competition: {
    title: "不是更复杂的链接页，而是中文经营入口的基础设施",
    body: "Link168 的差异不是堆叠概念，而是把中文场景、公开主页、二维码、后台治理和可扩展 AI 架构放在同一产品中。",
  },
  progress: {
    title: "已完成、内测中和下一阶段明确分开",
    body: "评委看到的状态来自当前产品边界：能打开和操作的标记为已完成，需要配置或仍在验证的标记为内测，尚未开发完成的进入下一阶段。",
  },
};
...
const FIXED_LISTS: Partial<Record<ShowcaseV2SectionKey, Array<{ title: string; description: string }>>> = {
  painPoints: [
    ...
    { title: "小团队缺少数字化能力", description: "创作者和小商家通常没有产品、设计、运营和技术团队。" },
  ],
  solution: [
  ...
  competition: [
    ...
    { title: "后台治理与安全边界", description: "包含邮箱验证、举报、封禁、日志和配置管理。" },
    { title: "AI 架构可扩展", description: "共享资料库底座配合不同 Agent 的提示词、模板和权限。" },
  ],
};

const PRODUCT_LINKS = [
  { name: "产品首页", note: "产品定位、注册入口和手机预览", href: "/", status: "已完成" },
...
const PROGRESS = [
  { name: "已完成", note: "注册登录、邮箱验证、主页编辑、公开主页、链接、二维码、后台治理" },
  { name: "内测中", note: "企业 AI、五个比赛演示 Agent、访问分析和企业资料能力" },
  { name: "下一阶段", note: "销售顾问 Agent、会员支付、客户线索和经营自动化" },
];
...
export default function ShowcaseExperience() {
  ...
  const openingCopy = COPY.opening;
    ...
    <div className="ui-page">
      <header className="sticky top-0 z-40 border-b border-[var(--ui-line)] bg-[var(--ui-surface)] backdrop-blur">
        <div className="ui-container flex min-h-16 items-center justify-between gap-4">
          <a href="#competition-top" className="flex items-center gap-3"><span className="grid size-9 place-items-center rounded-[var(--ui-radius-sm)] bg-[var(--ui-brand)] font-black text-white">L</span><span><strong className="block text-sm">Link168 比赛展示</strong><small className="text-[var(--ui-muted)]">评委专用 · 真实产品与进展</small></span></a>
          <nav className="hidden items-center gap-5 text-sm font-bold text-[var(--ui-muted)] xl:flex">{NAV_KEYS.map((key) => map.has(key) ? <a key={key} href={`#${anchor(key)}`} className="hover:text-[var(--ui-brand)]">{map.get(key)?.label}</a> : null)}</nav>
      ...
      <main id="competition-top">
        <section className="border-b border-[var(--ui-line)] bg-[var(--ui-surface)] py-14 sm:py-20">
          <div className="ui-container grid gap-10 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-center">
            <div><p className="ui-eyebrow">{opening?.eyebrow || "Link168 比赛展示"}</p><h1 className="ui-title mt-4 max-w-4xl text-4xl leading-[1.12] sm:text-5xl lg:text-[58px]">{openingCopy?.title}</h1><p className="ui-muted mt-6 max-w-2xl text-base leading-8 sm:text-lg">{openingCopy?.body}</p><div className="mt-8 flex flex-col gap-3 sm:flex-row"><a href={`#${anchor("productDemo")}`} className="ui-button-primary min-h-12 px-6 text-base">查看真实产品 <ArrowRight className="size-4" /></a><a href={`#${anchor("progress")}`} className="ui-button-secondary min-h-12 px-6 text-base">查看真实进展</a></div></div>
            <aside className="ui-surface divide-y divide-[var(--ui-line)] overflow-hidden"><div className="px-5 py-4 text-xs font-black text-[var(--ui-muted)]">当前展示状态</div>{[["核心主页闭环","已完成","success"],["邮箱验证与找回密码","已完成","success"],["AI 助理","内测中","warning"],["正式支付","暂未开放","muted"]].map(([name,status,tone]) => <div key={name} className="flex items-center justify-between gap-4 px-5 py-4"><span className="text-sm font-bold">{name}</span><span className={`rounded-full px-3 py-1 text-xs font-black ${tone === "success" ? "bg-[var(--ui-success-soft)] text-[var(--ui-success)]" : tone === "warning" ? "bg-[var(--ui-accent-soft)] text-[#7D5B24]" : "bg-[var(--ui-surface-muted)] text-[var(--ui-muted)]"}`}>{status}</span></div>)}</aside>
          </div>
        </section>
        ...
        <section className="bg-[var(--ui-ink)] py-14 text-white sm:py-16"><div className="ui-container flex flex-col gap-7 lg:flex-row lg:items-center lg:justify-between"><div><p className="text-xs font-black tracking-[0.12em] text-white/60">{ending?.eyebrow || "感谢评委"}</p><h2 className="mt-3 text-3xl font-black sm:text-4xl">Link168：把名片变成经营入口</h2><p className="mt-4 max-w-2xl leading-7 text-white/70">欢迎继续查看真实产品和当前可操作功能。</p></div><Link href="/" target="_blank" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-[var(--ui-radius-sm)] bg-white px-6 font-black text-[var(--ui-ink)]">进入真实产品 <ExternalLink className="size-4" /></Link></div></section>
      </main>
