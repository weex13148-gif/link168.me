    ...
    <div className="grid gap-2 lg:col-span-2">
      ...
      <div className="flex flex-wrap gap-2">
        {ICON_PRESETS.map((icon) => (
          <button key={icon} type="button" onClick={() => onPick(icon)} className={`grid size-10 place-items-center rounded-xl border text-lg ${value === icon ? "border-[var(--ui-brand)] bg-[var(--ui-brand-soft)]" : "border-[var(--ui-line)] bg-[var(--ui-surface-strong)]"}`}>{icon}</button>
        ))}
      </div>
    </div>
  );
  ...
  return (
    <>
      ...
      <div className="grid gap-2">
        ...
        <div className="grid size-12 place-items-center rounded-xl border border-[var(--ui-line)] bg-[var(--ui-surface-strong)]">
      ...
      {mode === "platform" ? (
        <div className="grid gap-2 lg:col-span-2">
          ...
          <div className="rounded-xl border border-[var(--ui-line)] bg-[var(--ui-surface-strong)] p-3 text-sm ui-muted">
            {url ? (
...
function ImageUploadField({
  ...
  return (
    <div className="grid gap-2 lg:col-span-2">
      ...
      {value ? (
        <div className="mt-2 grid size-24 place-items-center overflow-hidden rounded-xl border border-[var(--ui-line)] bg-[var(--ui-surface-strong)]">
    ...
    <div className="grid gap-3 lg:col-span-2">
      ...
      <div className="grid gap-3">
        {images.map((img, index) => (
          <div key={index} className="rounded-xl border border-[var(--ui-line)] bg-[var(--ui-surface-strong)] p-3">
    ...
    <>
      ...
      {ct === "booking" ? (
        <>
          ...
          <label className="flex items-center gap-2 rounded-xl border border-[var(--ui-line)] bg-[var(--ui-surface-strong)] px-3 py-2 lg:col-span-2">
            <input
              type="checkbox"
              ...
              <article key={link.id} className="bg-[var(--ui-surface)]">
                <div className="flex min-h-[56px] items-center gap-1 px-2 py-2 sm:min-h-16 sm:gap-3 sm:px-5">
                  ...
                  <div className="flex items-center gap-0.5 sm:gap-1">
                    <button type="button" onClick={() => void onMove(link.id, "up")} disabled={index === 0 || busy} className="grid size-[36px] place-items-center rounded-lg border border-[var(--ui-line)] bg-[var(--ui-surface-strong)] text-[var(--ui-muted)] disabled:opacity-30 sm:size-9" title="上移"><ChevronUp className="size-4" /></button>
                    <button type="button" onClick={() => void onMove(link.id, "down")} disabled={index === orderedLinks.length - 1 || busy} className="grid size-[36px] place-items-center rounded-lg border border-[var(--ui-line)] bg-[var(--ui-surface-strong)] text-[var(--ui-muted)] disabled:opacity-30 sm:size-9" title="下移"><ChevronDown className="size-4" /></button>
                    <button type="button" onClick={() => void onToggle(link)} disabled={busy} className={`grid size-[36px] place-items-center rounded-lg border ${link.is_active ? "border-[var(--ui-brand)] bg-[var(--ui-brand)] text-white" : "border-[var(--ui-line)] bg-[var(--ui-surface-strong)] text-[var(--ui-muted)]"} sm:size-9`} title={link.is_active ? "点击隐藏" : "点击公开"}>
                      {busy ? <Loader2 className="size-4 animate-spin" /> : link.is_active ? <Eye className="size-4" /> : <EyeOff className="size-4" />}
                    </button>
                    <button type="button" onClick={() => setExpandedId(expanded ? "" : link.id)} className="grid size-[36px] place-items-center rounded-lg border border-[var(--ui-line)] bg-[var(--ui-surface-strong)] text-[var(--ui-muted)] sm:size-9" title="编辑"><Pencil className="size-4" /></button>
                  </div>
                </div>
