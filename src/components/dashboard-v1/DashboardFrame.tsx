...
function SaveStatus({ state, mobile = false }: { state: SaveState; mobile?: boolean }) {
  const config = {
    ...
    dirty: { text: "未保存", dot: "bg-[var(--ui-accent)]", className: "bg-[var(--ui-accent-soft)] text-[var(--ui-accent)]" },
      ...
      <header className="sticky top-0 z-40 border-b border-[var(--ui-line)] bg-[color:var(--ui-surface)]/96 backdrop-blur">
        <div className="ui-admin-container flex min-h-14 items-center justify-between gap-2 sm:min-h-16 sm:gap-4">
          ...
          <div className="flex min-w-0 items-center gap-1.5 sm:gap-2">
            ...
            <button type="button" onClick={() => selectTab("account")} className="hidden min-h-9 items-center gap-2 rounded-xl border border-[var(--ui-line)] bg-[var(--ui-surface-strong)] px-3 text-xs font-black text-[var(--ui-muted)] sm:inline-flex">
            ...
            <button type="button" onClick={onLogout} className="hidden shrink-0 place-items-center rounded-xl border border-[var(--ui-line)] bg-[var(--ui-surface-strong)] text-[var(--ui-muted)] hover:text-[var(--ui-danger)] sm:grid sm:size-10" title="退出登录" aria-label="退出登录">
            ...
            {previewOpen ? (
              <div className="ui-surface relative p-4">
                <div className="mb-3 flex items-center justify-between gap-3">
                  ...
                  <button type="button" onClick={() => setPreviewOpen(false)} className="grid size-9 place-items-center rounded-xl border border-[var(--ui-line)] bg-[var(--ui-surface-strong)] text-[var(--ui-muted)]" aria-label="收起预览" title="收起预览">
              ...
              <button type="button" onClick={() => setPreviewOpen(true)} className="grid min-h-36 w-11 place-items-center rounded-xl border border-[var(--ui-line)] bg-[var(--ui-surface-strong)] text-[var(--ui-brand)] shadow-[var(--ui-shadow-sm)]" aria-label="展开预览" title="展开预览">
        ...
        <div className="fixed inset-0 z-50 flex items-end bg-black/35 p-3 lg:hidden" onClick={() => setMoreOpen(false)} role="presentation">
          <section className="w-full rounded-[24px] bg-[var(--ui-surface)] p-4 shadow-[var(--ui-shadow-lg)]" onClick={(event) => event.stopPropagation()} role="dialog" aria-modal="true" aria-label="更多功能">
            ...
            <div className="mt-4 grid gap-3">
              <button type="button" onClick={() => { setMoreOpen(false); goToNotifications(); }} className="relative flex min-h-[48px] items-center gap-3 rounded-xl border border-[var(--ui-line)] bg-[var(--ui-surface-strong)] px-4 text-left font-black transition hover:bg-[var(--ui-surface-muted)]">
              ...
              <button type="button" onClick={() => selectTab("appearance")} className="flex min-h-[48px] items-center gap-3 rounded-xl border border-[var(--ui-line)] bg-[var(--ui-surface-strong)] px-4 text-left font-black transition hover:bg-[var(--ui-surface-muted)]"><Palette className="size-5 text-[var(--ui-brand)]" />主题装修</button>
              <button type="button" onClick={() => selectTab("share")} className="flex min-h-[48px] items-center gap-3 rounded-xl border border-[var(--ui-line)] bg-[var(--ui-surface-strong)] px-4 text-left font-black transition hover:bg-[var(--ui-surface-muted)]"><QrCode className="size-5 text-[var(--ui-brand)]" />分享与二维码</button>
              <button type="button" onClick={() => selectTab("account")} className="flex min-h-[48px] items-center gap-3 rounded-xl border border-[var(--ui-line)] bg-[var(--ui-surface-strong)] px-4 text-left font-black transition hover:bg-[var(--ui-surface-muted)]"><ShieldCheck className="size-5 text-[var(--ui-brand)]" />账户与安全</button>
              <button type="button" onClick={onLogout} className="flex min-h-[48px] items-center gap-3 rounded-xl border border-[var(--ui-line)] bg-[var(--ui-surface-strong)] px-4 text-left font-black text-[var(--ui-danger)] transition hover:bg-[var(--ui-danger-soft)]"><LogOut className="size-5" />退出登录</button>
            </div>
          </section>
        </div>
      ) : null}
