    ...
    <div className="grid gap-5">
      ...
      <div className="grid gap-5 xl:grid-cols-[1fr_0.9fr]">
        <section className="ui-surface p-5 sm:p-6">
          ...
          <div className="mt-5 grid gap-2">
            {checklist.map((item) => (
              <button key={item.label} type="button" onClick={() => onNavigate(item.tab)} className="flex min-h-12 items-center gap-3 rounded-xl border border-[var(--ui-line)] bg-[var(--ui-surface-strong)] px-4 text-left transition hover:border-[color:var(--ui-brand)]/35">
        ...
        <section className="ui-surface p-5 sm:p-6">
          ...
          <div className="mt-5 grid gap-2">
            {recentLinks.length ? recentLinks.map((link) => (
              <div key={link.id} className="flex min-h-12 items-center gap-3 rounded-xl border border-[var(--ui-line)] bg-[var(--ui-surface-strong)] px-4">
              ...
              <button type="button" onClick={() => onNavigate("links")} className="grid min-h-36 place-items-center rounded-xl border border-dashed border-[var(--ui-line)] bg-[var(--ui-surface-strong)] text-center">
      ...
      <section className="grid gap-3 sm:grid-cols-3">
        ...
        <button type="button" onClick={() => onNavigate("appearance")} className="ui-surface flex min-h-20 items-center gap-3 p-4 text-left transition hover:-translate-y-0.5">
          <span className="grid size-10 place-items-center rounded-xl bg-[var(--ui-accent-soft)] text-[var(--ui-accent)]"><Palette className="size-5" /></span>
