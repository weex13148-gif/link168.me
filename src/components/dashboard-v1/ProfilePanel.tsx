    ...
    <div className="grid gap-5">
      ...
      <form onSubmit={onSave} className="ui-surface p-5 sm:p-6">
        <div className="grid gap-7 xl:grid-cols-[220px_minmax(0,1fr)]">
          ...
          <div className="grid gap-5">
            <label className="grid gap-2"><span className="text-sm font-black">公开主页地址</span><div className={`flex min-h-12 items-center rounded-xl border bg-[var(--ui-surface-strong)] px-3 ${canSetUsername ? "border-[var(--ui-line)] focus-within:border-[var(--ui-brand)] focus-within:ring-4 focus-within:ring-[color:var(--ui-brand)]/10" : "border-[var(--ui-line)] bg-[var(--ui-surface-muted)]"}`}><span className="shrink-0 text-sm font-bold text-[var(--ui-muted)]">link168.me/</span><input value={username} onChange={(event) => onUsernameChange(event.target.value.replace(/[^a-z0-9_-]/gi, "").toLowerCase())} placeholder="例如：abao" disabled={!canSetUsername} maxLength={32} className="min-w-0 flex-1 bg-transparent px-1.5 text-sm font-black outline-none disabled:text-[var(--ui-muted)]" /></div><span className="text-xs leading-5 ui-muted">{canSetUsername ? "仅支持小写字母、数字、下划线和短横线。首次保存后地址会锁定，避免旧链接失效。" : "公开地址已锁定。需要变更时请通过账户页面联系平台处理。"}</span></label>
            ...
            <label className="grid gap-2"><span className="text-sm font-black">个人简介</span><textarea value={bio} onChange={(event) => onBioChange(event.target.value)} maxLength={160} rows={5} placeholder="用一两句话介绍你提供的内容、服务或联系方式。" className="rounded-xl border border-[var(--ui-line)] bg-[var(--ui-surface-strong)] p-3.5 text-sm leading-6 outline-none transition focus:border-[var(--ui-brand)] focus:ring-4 focus:ring-[color:var(--ui-brand)]/10" /><span className="text-right text-xs ui-muted">{bio.length}/160</span></label>
