...
export default function DashboardV1Client() {
  ...
  else if (activeTab === "appearance") panel = <AppearancePanel theme={core.profile?.theme || "Link168 草木默认"} template={core.profile?.template || "business"} customThemes={core.planEntitlements.customThemes} saving={core.appearanceSaving} onSave={core.saveAppearance} onUpgrade={onUpgrade} />;
  ...
  else if (activeTab === "account") panel = <AccountPanel user={core.user} planLabel={core.planEntitlements.planLabel} isPaid={core.planEntitlements.isPaid} canUpgrade={core.planEntitlements.canUpgrade} sessions={account.sessions} sessionsLoading={account.sessionsLoading} resendingEmail={account.resendingEmail} passwordSaving={account.passwordSaving} onResendEmail={account.resendEmail} onChangePassword={account.changePassword} onRevokeSession={account.revokeSession} onRevokeOthers={account.revokeOthers} onUpgrade={onUpgrade} onLogout={() => void logout()} />;
