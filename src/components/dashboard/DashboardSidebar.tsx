    ...
    <>
      <nav className="ui-surface hidden h-full w-full flex-col gap-1 p-4 lg:flex">
        <div className="mb-3 flex items-center gap-3 px-2 pt-1">
          <div className="grid size-10 place-items-center rounded-[var(--ui-radius-sm)] bg-[var(--ui-brand)] text-[var(--ui-surface)]">
            ...
            return (
              <button
                ...
                className={`flex min-h-[62px] items-start gap-3 rounded-[var(--ui-radius-sm)] px-3 py-3 text-left transition ${
                  active
                    ? "bg-[var(--ui-brand)] text-[var(--ui-surface)]"
                    : "text-[var(--ui-ink)] hover:bg-[var(--ui-surface-muted)]"
                }`}
              >
                <span className={`grid size-9 shrink-0 place-items-center rounded-[10px] ${active ? "bg-[var(--ui-surface)]/15 text-[var(--ui-surface)]" : "bg-[var(--ui-brand-soft)] text-[var(--ui-brand)]"}`}>
                  <Icon aria-hidden className="size-5" />
                </span>
                <span className="min-w-0">
                  <span className="block text-sm font-black">{label}</span>
                  <span className={`mt-0.5 block text-xs leading-5 ${active ? "text-[var(--ui-surface)]/80" : "text-[var(--ui-muted)]"}`}>{description}</span>
                </span>
              </button>
            );
          })}
